...
from reels import download_and_analyze_reels, VisualSegmentAnalysis
...
class TestDownloadAndAnalyzeWorkflow(unittest.TestCase):
    ...
    @patch('builtins.open', new_callable=mock_open,
           read_data="http://example.com/reel1")
    def test_full_workflow(self, mock_open_file, mock_exists,
                           mock_subprocess, mock_analyze_video,
                           mock_write_csv, mock_makedirs):
        # --- ARRANGE ---
        # 1. Simulate the results for all os.path.exists checks in
        # of execution
        # - reels folder: False (in create_folders)
        # - transcripts folder: False (in create_folders)
        # - master_analysis.csv: False (doesn't exist)
        # - dummy_links.txt: True (so the function doesn't exit
        ...
        mock_analysis_result = [
            VisualSegmentAnalysis(
                ...
                segment_id="1", start_time="00:00:00.000",
                ...
                effectiveness_justification="Very cute."
            )
        ]
        ...
        mock_subprocess.assert_any_call(
            ['yt-dlp', '--get-filename', '-o', 'reels/%(title)s.%(ext)s',
             "http://example.com/reel1"],
            capture_output=True, text=True, check=True
        )

        # 2. Check if it tried to download the video since it didn't
        mock_subprocess.assert_any_call(
            ['yt-dlp', '-o', 'reels/%(title)s.%(ext)s',
             "http://example.com/reel1"],
            check=True
        )
        ...
        # 4. Check if it tried to write the final results to the
        # master CSV
        # The first argument to the first call of write_analysis_to_csv
        mock_write_csv.assert_called_once()
        self.assertEqual(len(mock_write_csv.call_args[0][0]), 1)  # List
        self.assertEqual(mock_write_csv.call_args[0][0][0].video_filename,
                         "Test-Video.mp4")
        self.assertEqual(mock_write_csv.call_args[0][1],
                         "master_analysis.csv")  # The path
    ...
    @patch('reels.os.path.exists')
    @patch('builtins.open')
    def test_iterates_through_multiple_links_and_skips_analyzed(
            self, mock_open_file, mock_exists, mock_subprocess,
            mock_analyze_video, mock_write_csv, mock_makedirs):
        # --- ARRANGE ---

        # 1. Simulate file contents using proper multi-line strings
        links_data = "http://example.com/reel1\\nhttp://example.com/reel2"
        master_csv_data = (
            '"video_filename","segment_id","start_time","end_time",'
            '"shot_type","spoken_text","visual_description",'
            '"inferred_purpose","effectiveness_rating",'
            '"effectiveness_justification"\n'
            '"reel1.mp4","1","00:00:00.000","00:00:05.000","B-roll",'
            '"Hello","A scene","To show something","5","Good"\n'
        )

        # 2. Mock `open()` to return the correct file data when
        mock_open_file.side_effect = [
            mock_open(read_data=master_csv_data).return_value,
            mock_open(read_data=links_data).return_value
        ]

        # 3. Mock `os.path.exists()` for the sequence of checks in
        # the script.
